...
describe Enumerable do
  ...
  describe '#my_each_with_index' do
    ...
    context 'if block given, checks my_each method' do
      ...
      it 'should return each item as requested with indexes' do
        actual = empty_array
        array_with_strings.my_each_with_index { |element, index| actual.push("[#{element}]'s index is #{index}") }
        expect(actual).to eql(["[one]'s index is 0", "[two]'s index is 1", "[three]'s index is 2", "[four]'s index is 3", "[five]'s index is 4"]) # rubocop:disable Layout/LineLength
      end
    end
  end
  ...
  describe '#my_any?' do
    context 'When passed block with at least one condition met' do
      it 'returns true if any condition is true' do
        expect(array_with_strings_and_numbers.my_any?(Integer)).to be true
      end

      it 'returns true if any condition of array is true' do
        expect(array_with_numbers_and_nil.my_any?(nil)).to be true
      end

      it 'returns true if any condition of block is true' do
  ...
  describe '#my_inject' do
    context 'when only operator is given' do
      it 'should return cumulation of all elements affected by operator' do
        expect((1..5).my_inject(:+)).to eql(15)
      end
    end

    context 'when argument and operator is given' do
      it 'should return cumulation of all elements affected by operator and initial value given as an argument' do
        expect((1..5).my_inject(10, :+)).to eql(25)
      end
    end

    context 'block is given' do
      it 'should return cumulation of all elements' do
        expect(array_with_numbers.my_inject { |memo, element| memo * element }).to eql(120)
      end
    end

    context 'when initial value and block is given' do
      it 'should return cumulation of all elements and initial value' do
        expect(array_with_numbers.my_inject(10) { |memo, element| memo * element }).to eql(1200)
      end
    end
  end
end
